...
void trie_insert(trie_node_t *root, const char *word, int doc_id, int page_num,
    ...
    DEBUG_PRINT("[DEBUG INSERT] word='%s' doc=%d page=%d offset=%ld\n", word,
                doc_id, page_num, byte_offset);
...
void add_occurence_to_node(trie_node_t *node, int doc_id, int page_num,
    ...
    word_occurrence_t *new_occ =
        create_occurrence(doc_id, page_num, byte_offset);
    if (new_occ == NULL)
        return;
